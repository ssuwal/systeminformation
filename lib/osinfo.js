 ...
 * action - 'scan', 'update' [Update Virus Definition]
 *  */

function defender(action) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = {
        AMEngineVersion: '',
        ...
        // ComputerID: 0BACA7B8-E8C6-4EAC-95D4-6DAF573D7668
        // ComputerState: 0
        ...
        // QuickScanAge: 1
        QuickScanEndTime: '',
        // QuickScanStartTime: 20210107023249.757000+000
      ...
      try {
        ...
        util.wmic('/Namespace:\\\\root\\microsoft\\windows\\defender path MSFT_MpComputerStatus get /value')
          .then( (stdout) => {
            let lines = stdout.toString().split('\r\n');
            const vals = lines.reduce((acc, curr) => {
              const splt = curr.split('=').map(n => n.trim());
              if (!KeyMap.includes(splt[0])) {
                return acc;
              }
              const processedVal = /^TRUE$/i.test(splt[1]) ? true :
                                      /^FALSE$/i.test(splt[1]) ? false :
                                      /(Updated|Time)$/i.test(splt[0]) && splt[1] ? getDate(splt[1]) : splt[1];
              acc.set(splt[0], processedVal);
              return acc;
            }, new Map());
            resolve(Object.fromEntries(vals));
          })
      } catch (e) {
        console.error(e);
        resolve(result);
...
function getDate(dateStr) {
  ...
  const dateArr = dateStr.substring(4).match(/.{1,2}/g);
  dateArr[0] = dateArr[0] - 1;
  return new Date(year, ...dateArr).toLocaleDateString('en-US', util.dateOpts);
}
